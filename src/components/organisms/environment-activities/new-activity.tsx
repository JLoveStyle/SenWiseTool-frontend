...
export function NewActivityEnvironment() {
  ...
  const handleCreateActivity = async (
    ...
    console.log({ formData, uploadedURLs });
    const dataToDB = {
    ...
    const existingActivities = LOCAL_STORAGE.get("environments") ?? [];
    LOCAL_STORAGE.save("environments", [
      ...existingActivities,
      { id: existingActivities.length + 1, ...dataToDB },
    ]);

    toast.success("Activité d'environment créée avec succès");
    setIsLoading(false);
    router.refresh();
    router.push(Route.environment);
    return;
  };
  ...
  const handleUploadPictures = async (formData: ActivityFormProps) => {
    if (formData.pictures_url && formData.pictures_url.length !== 0) {
      const { data, error } = await UpdateFilesToS3({
        ...
        directory: "environment/pictures",
      });
  ...
  const handleUploadDocuments = async (formData: ActivityFormProps) => {
    if (formData.documents_url && formData.documents_url.length !== 0) {
      const { data, error } = await UpdateFilesToS3({
        ...
        directory: "environment/documents",
      });
      if (error) {
        console.log(error);
